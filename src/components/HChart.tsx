...
export default function HChart(){
    ...
    useEffect(() => {
        console.log("Atomを使うFormの現在の入力値は",prefState)
        const PrefPopulations = fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL ? process.env.NEXT_PUBLIC_VERCEL_URL : "http://localhost:3000"}/api/population?prefcodes=${[...prefState.map(pref => {return pref.prefCode})].join(",")}` , {method: "POST",next: {revalidate: 3600}}).then(res => res.json()).then((PrefPopulationData) => {
